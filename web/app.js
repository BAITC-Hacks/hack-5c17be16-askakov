...
function showGraph() {
  ...
  if (viewMode === 'path') {
    ...
    // При выборе узла показываем и соседей из других кластеров.
...
async function init(){
  ...
  if(m.period_start && m.period_end){$('period-range').textContent=`Период данных: ${displayDate(m.period_start)} — ${displayDate(m.period_end)}`;$('period').hidden=false;}
