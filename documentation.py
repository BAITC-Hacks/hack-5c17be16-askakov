...
def configuration_section():
    ...
    table = '\n'.join(f'| `{role}` | {rule} | `{scores[role]}` |' for role, rule in role_rules().items())
    return f'''{START}
## Метрики и роли
...
Если исходящие ≥{e['min_out_kzt']:,} ₸ и выход **строго больше {e['max_pass_through']} × вход**, у любой роли добавляется «Источник средств вне выборки». Нулевой вход также учитывается. Только для peripheral с этим признаком `role_priority_multiplier={p['external_peripheral_multiplier']}`; иначе он равен 1. Только для seed с ролью {', '.join(p['seed_discount_roles'])} `seed_priority_multiplier={p['seed_multiplier']}` и в why добавляется «Клиент уже известен — приоритет снижен». Seed с ролью consolidator/distributor/coordinator сохраняют множитель 1 и получают пояснение «Клиент уже известен правоохранителям, но является точкой сбора/раздачи — ключ к уровню выше». Для остальных узлов множитель равен 1. `role_score` отдельно на приоритет не умножается.

Компоненты и перцентили до множителей сохраняются в `features_debug.csv`: `priority_component_*`, `priority_percentile_*`; сумма вкладов равна `base_priority_score`. Вес посредничества в приоритете равен {w['betweenness']}: отсутствие исходящих у конечной точки сбора не обнуляет отдельную долю её оценки. Betweenness сохраняется в подтверждении role_score координатора и отладочных метриках.

Приёмка проверяет среди **всех узлов** общий ранг ≤{review['max_rank_all']} для каждого узла с итоговой ролью consolidator, ≥{review['min_payers']} плательщиками и входом ≥{review['min_in_kzt']:,} ₸. Это проверяемый критерий на текущей выборке, а не дополнительный бонус или ручное перемещение узлов. При изменении данных фиксированные веса могут перестать выполнять критерий; тогда verify.py сообщает об этом.
