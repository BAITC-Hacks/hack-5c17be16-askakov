...
Проверка после аудита: 2 248 узлов, 3 119 рёбер, 4 840 операций, 88 кластеров. Роли: coordinator — 12, consolidator — 35, distributor — 68, transit — 66, terminal — 924, peripheral — 1 143. После пересмотра приоритета в топ-20 — 0 peripheral и 4 seed-хаба. Это наблюдаемый результат на предоставленных данных, а не квоты алгоритма. Полный пересчёт занимает около 7 секунд на текущей машине.

Отчёты каждого пункта с изменениями «было → стало», распределением и полным топ-20 находятся в `output/audit/stepN/report.md`; точный diff — в `changes.diff`, выгрузки до и после — в подкаталогах `before/after`. Отчёт первого аудита — в [AUDIT_REPORT.md](AUDIT_REPORT.md), последующие три правки приоритета — в [PRIORITY_REPORT.md](PRIORITY_REPORT.md).
